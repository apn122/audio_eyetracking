...
function make_slides(f) {
  ...
  slides.trial = slide({
    ...
    present_handle : function(stim) {
      ...
      $(".loc").bind("click",function(e){
        e.preventDefault();
        exp.coordinates = e.clientX + ':' + e.clientY;
    ...
    button : function() {
      ...
      console.log("Selection: ",exp.selection);
      console.log("Timer: ",exp.timer);
      console.log("Coordinates: ", exp.coordinates);
      this.log_responses();
      _stream.apply(this); /* use _stream.apply(this); if and only if there is
      "present" data. (and only *after* responses are logged) */
...
function init_explogic() {
  ...
  exp.display_imgs = function(stim){
    ...
    $(".loc9").html(girl);
    $(".loc10").html(girl);
    //exp.get_image_coordinates();
  };

  // exp.get_image_coordinates = function(){
  //   //var one = document.getElementsByClassName("loc1");
  ...
  //   w = rect.width;
  //   h = rect.height;
  //   var size = "Left: " + x + ", Top: " + y + ", Width: " + w + ", Height: " + h;
  //   console.log("Size of A0I1: ", size)
  // }
